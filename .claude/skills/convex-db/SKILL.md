...
name: convex-db
description: >
  Integrate Convex database into web apps with best practices for real-time sync,
  type-safe schemas, and reactive queries. Covers schema design, function patterns
  (queries/mutations/actions), React/Next.js integration, authentication with Clerk,
  file storage, and scheduling. Trigger when users build with Convex or ask about
  Convex database patterns, gotchas, or integration.
---
