## GridCombatScene.gd
## Grid Combat v1: Unified 8x4 battlefield with both teams on one grid.
## Copy of CombatScene.gd adapted for the new grid layout.
##
## Source: Grid Combat System v1, Phase 3
## Grid layout: Cols 0-1 (Player), 2-5 (Open), 6-7 (Enemy), Rows 0-3
extends Control
...
const KENNEY_FUTURE_FONT = preload("res://Assets/UI/Kenney/UI/Font/Kenney Future.ttf")
...
# Grid Combat v1: Unified 8x4 grid (both teams share one battlefield)
@onready var battlefield: VBoxContainer = $MainContent/Battlefield
...
# Class Card Backgrounds: texture cache (class_id -> Texture2D or null)
...
# ============================================================================
# GRID COMBAT v1: TWEEN ANIMATIONS
# ============================================================================

# Tween tracking: active tweens per unit for cleanup
var _unit_tweens: Dictionary = {}  # unit_id -> Array[Tween]
var _idle_tweens: Dictionary = {}  # unit_id -> Tween (idle bob)
var _unit_animators: Dictionary = {}  # unit_id -> HeroSpriteAnimator (frame-by-frame sprite animation)
...
var _btn_move: Button = null  # Grid Combat v1: Free movement button
...
# Grid Combat v1: Movement tile selection state
var _move_selection_active: bool = false
var _valid_move_tiles: Array = []  # Array of Vector2i
var _move_tile_highlights: Array = []  # Array of Controls (overlay highlights)

# Phase 8: Hero Placement Phase
var _placement_phase_active: bool = false
var _selected_placement_unit_id: String = ""
var _start_battle_btn: Button = null
var _placement_instruction: Label = null
var _placement_highlights: Array = []  # Array of Controls for placement tile overlays
...
# ============================================================================
# GRID COMBAT v1: Unified 8x4 Grid
# ============================================================================
# Layout: [P-1|P-2|P-3|P-4|E-4|E-3|E-2|E-1]
# Cols 0-1: Player zone, Cols 2-5: Open battlefield, Cols 6-7: Enemy zone
# Rows 0-3: Vertical lanes
# pos.x (from snapshot) maps to UI COLUMN (0-7)
# pos.y (from snapshot) maps to UI ROW (0-3)

const GRID_COLS: int = 8   # Unified grid width
const GRID_ROWS: int = 4   # Unified grid height


## Get a cell container by row and column on the unified grid.
## ui_row: 0-3, ui_col: 0-7
func _get_cell_at(ui_row: int, ui_col: int) -> PanelContainer:
	var row_clamped = clampi(ui_row, 0, GRID_ROWS - 1)
	var col_clamped = clampi(ui_col, 0, GRID_COLS - 1)
	var row_name = "GridRow_%d" % row_clamped
	var cell_name = "Cell_c%d" % col_clamped
	return battlefield.get_node("%s/%s" % [row_name, cell_name]) as PanelContainer


## Legacy compatibility wrapper: _get_cell(team, row, col) -> maps to unified grid.
## Party team: col offset 0, Enemy team: col offset 0 (positions are already unified).
func _get_cell(team: String, ui_row: int, ui_col: int) -> PanelContainer:
	# In the unified grid, the pos.x from snapshot IS the column directly
	# This wrapper exists for any remaining legacy call sites
	return _get_cell_at(ui_row, ui_col)


## Clear all cell contents on the entire unified grid.
func _clear_all_slots() -> void:
	for ui_row in range(GRID_ROWS):
		var row_node = battlefield.get_node_or_null("GridRow_%d" % ui_row)
		if row_node == null:
			continue
...
## Legacy compatibility: _clear_team_slots just clears the full grid.
func _clear_team_slots(_team: String) -> void:
	_clear_all_slots()


## Apply zone-tinted backgrounds to grid cells.
## Player zone (cols 0-3): subtle blue, Enemy zone (cols 4-7): subtle red, center cols lighter.
func _clear_cell_backgrounds() -> void:
	for ui_row in range(GRID_ROWS):
		...
		for ui_col in range(GRID_COLS):
			...
			if cell == null:
				continue
			# Fixed cell size for uniform 8x4 grid
			cell.custom_minimum_size = Vector2(120, 140)
			cell.clip_contents = true
			var style = StyleBoxFlat.new()
			if ui_col <= 1:
				# Player home zone — deeper blue tint
				style.bg_color = Color(0.12, 0.18, 0.30, 0.35)
			elif ui_col <= 3:
				# Player front zone — lighter blue tint
				style.bg_color = Color(0.12, 0.18, 0.30, 0.22)
			elif ui_col <= 5:
				# Enemy front zone — lighter red tint
				style.bg_color = Color(0.30, 0.12, 0.12, 0.22)
			else:
				# Enemy home zone — deeper red tint
				style.bg_color = Color(0.30, 0.12, 0.12, 0.35)
			style.set_border_width_all(1)
			style.border_color = Color(0.4, 0.35, 0.3, 0.3)
...
func _create_empty_slot_placeholder() -> Control:
	...
	placeholder.color = Color(0, 0, 0, 0)  # Transparent — cell bg shows through
...
## Create a fog of war placeholder for hidden grid cells during placement phase.
func _create_fog_placeholder(col: int) -> Control:
	var fog = ColorRect.new()
	fog.name = "FogPlaceholder"
	fog.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	fog.size_flags_vertical = Control.SIZE_EXPAND_FILL
	fog.mouse_filter = Control.MOUSE_FILTER_IGNORE
	# Darker fog on enemy zone, lighter on open battlefield
	if col >= 6:
		fog.color = Color(0.05, 0.03, 0.08, 0.85)
	else:
		fog.color = Color(0.08, 0.06, 0.10, 0.55)
	return fog
...
## Called when CombatController starts the placement phase.
func _on_placement_phase_started(_player_units: Array) -> void:
	_placement_phase_active = true
	_selected_placement_unit_id = ""
	print("[UI] Placement phase started — heroes can be repositioned")
	TutorialOverlay.try_show(self, "tutorial_grid_placement")

	# Hide combat buttons during placement
	...
	# Re-apply cell backgrounds (fog handled in _refresh_grid)
	_clear_cell_backgrounds()

	# Show only player units (enemies hidden by fog)
	var snapshot = _combat_controller.get_units_snapshot()
	_refresh_grid(snapshot["player"], snapshot["enemy"])

	# Create "Start Battle!" button
	_create_start_battle_button()

	# Create instruction label
	_create_placement_instruction()

	_log("[color=cyan]Position your heroes! Click a hero, then click a tile to move them.[/color]")
	_log("[color=cyan]Click two heroes to swap their positions.[/color]")


## Called when placement phase is confirmed — reveal enemies and start combat.
func _on_placement_confirmed() -> void:
	_placement_phase_active = false
	_selected_placement_unit_id = ""
	_clear_placement_highlights()
	print("[UI] Placement confirmed — revealing enemies and starting combat")

	# Remove start battle button and instruction
	if _start_battle_btn != null:
		_start_battle_btn.queue_free()
		_start_battle_btn = null
	if _placement_instruction != null:
		_placement_instruction.queue_free()
		_placement_instruction = null

	# Restore combat buttons
	auto_button.visible = true
	# step_button stays hidden (using player action buttons)

	# Re-apply cell backgrounds (fog removed)
	_clear_cell_backgrounds()

	# Full refresh with enemies now visible
	var snapshot = _combat_controller.get_units_snapshot()
	_refresh_grid(snapshot["player"], snapshot["enemy"])
	_refresh_all_panels()

	_log("[b]--- Battle Begins! ---[/b]")

	# Start first turn
	call_deferred("_step_turn")


## Create the "Start Battle!" button during placement phase.
func _create_start_battle_button() -> void:
	...
	_start_battle_btn.custom_minimum_size = Vector2(200, 50)
	_start_battle_btn.add_theme_font_size_override("font_size", 20)

	# Style the button prominently
	var style = StyleBoxFlat.new()
	style.bg_color = Color(0.15, 0.55, 0.2, 1.0)
	style.set_border_width_all(2)
	style.border_color = Color(0.3, 0.8, 0.3, 1.0)
	style.set_corner_radius_all(6)
	_start_battle_btn.add_theme_stylebox_override("normal", style)

	var hover_style = style.duplicate()
	hover_style.bg_color = Color(0.2, 0.65, 0.25, 1.0)
	_start_battle_btn.add_theme_stylebox_override("hover", hover_style)

	_start_battle_btn.add_theme_color_override("font_color", Color.WHITE)
	_start_battle_btn.pressed.connect(_on_start_battle_pressed)

	# Add to bottom panel area (near the buttons)
	var button_row = $BottomPanel/ButtonRow
	button_row.add_child(_start_battle_btn)


## Create the placement instruction label.
func _create_placement_instruction() -> void:
	if _placement_instruction != null:
		_placement_instruction.queue_free()

	_placement_instruction = Label.new()
	_placement_instruction.text = "Click a hero to select, then click a tile or another hero to reposition"
	_placement_instruction.add_theme_font_size_override("font_size", 13)
	_placement_instruction.add_theme_color_override("font_color", Color(0.7, 0.85, 1.0, 0.9))
	_placement_instruction.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER

	# Add above the battlefield
	var idx = battlefield.get_index()
	var parent = battlefield.get_parent()
	parent.add_child(_placement_instruction)
	parent.move_child(_placement_instruction, idx)
...
func _on_start_battle_pressed() -> void:
	if _combat_controller == null:
		return
	_combat_controller.confirm_placement()


## Handle clicking on a hero portrait during placement phase.
func _on_placement_unit_clicked(unit_id: String) -> void:
	...
	if _selected_placement_unit_id == "":
		# No unit selected yet — select this one
		_selected_placement_unit_id = unit_id
		_highlight_selected_placement_unit(unit_id)
		_show_placement_tile_options(unit_id)
		print("[UI] Placement: selected %s" % unit_id)
	elif _selected_placement_unit_id == unit_id:
		# Same unit clicked — deselect
		_selected_placement_unit_id = ""
		_clear_placement_highlights()
		print("[UI] Placement: deselected %s" % unit_id)
	else:
		# Different hero clicked — swap the two heroes
		var success = _combat_controller.submit_placement_move(_selected_placement_unit_id, _get_unit_grid_pos(unit_id))
		if success:
			print("[UI] Placement: swapped %s <-> %s" % [_selected_placement_unit_id, unit_id])
		_selected_placement_unit_id = ""
		_clear_placement_highlights()
...
## Handle clicking on an empty tile during placement phase.
func _on_placement_tile_clicked(tile_pos: Vector2i) -> void:
	if not _placement_phase_active:
		return
	if _selected_placement_unit_id == "":
		return

	# Move selected unit to this tile
	var success = _combat_controller.submit_placement_move(_selected_placement_unit_id, tile_pos)
	if success:
		print("[UI] Placement: moved %s to (%d,%d)" % [_selected_placement_unit_id, tile_pos.x, tile_pos.y])
	_selected_placement_unit_id = ""
	_clear_placement_highlights()
	# Refresh grid
	var snapshot = _combat_controller.get_units_snapshot()
...
## Get the grid position of a unit from the snapshot.
func _get_unit_grid_pos(unit_id: String) -> Vector2i:
	if _combat_controller == null:
		return Vector2i(-1, -1)
	var snapshot = _combat_controller.get_units_snapshot()
...
## Highlight the selected hero during placement.
func _highlight_selected_placement_unit(unit_id: String) -> void:
	var display = _unit_displays.get(unit_id)
	if display == null:
		return
	var highlight = display.get_node_or_null("HighlightFrame")
	if highlight != null:
		highlight.color = Color(0.2, 0.8, 1.0, 0.5)  # Bright cyan


## Show available placement tiles for the selected hero.
func _show_placement_tile_options(unit_id: String) -> void:
	_clear_placement_highlights()
	# Highlight all placement-allowed tiles (cols 0-3) that are empty or have a different hero
	for col in range(4):  # cols 0-3 (left half)
		for row in range(GRID_ROWS):
			var cell = _get_cell_at(row, col)
			if cell == null:
				continue
			var tile_pos = Vector2i(col, row)
			# Check if this tile has the selected unit (skip — already there)
			var current_pos = _get_unit_grid_pos(unit_id)
			if current_pos == tile_pos:
				continue
			# Add green highlight overlay
			var highlight = ColorRect.new()
			highlight.name = "PlacementHighlight"
			highlight.color = Color(0.1, 0.7, 0.2, 0.3)
			...
			highlight.gui_input.connect(_on_placement_highlight_input.bind(tile_pos))
			cell.add_child(highlight)
			_placement_highlights.append(highlight)


## Handle click on a placement highlight tile.
func _on_placement_highlight_input(event: InputEvent, tile_pos: Vector2i) -> void:
	if not (event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT):
		return
	_on_placement_tile_clicked(tile_pos)


## Handle click on an empty tile in the player zone during placement.
func _on_placement_empty_tile_input(event: InputEvent, tile_pos: Vector2i) -> void:
	if not (event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT):
		return
	_on_placement_tile_clicked(tile_pos)


## Clear all placement phase highlights.
func _clear_placement_highlights() -> void:
	for h in _placement_highlights:
		if is_instance_valid(h):
			h.queue_free()
	_placement_highlights.clear()
	# Reset highlight on previously selected unit
	for uid in _unit_displays:
		var display = _unit_displays[uid]
		if is_instance_valid(display):
			var highlight = display.get_node_or_null("HighlightFrame")
			if highlight != null:
				highlight.color = Color(1.0, 0.85, 0.0, 0.25)  # Back to normal
...
func _ready() -> void:
	...
	# Add Retreat button to ButtonRow (always available)
	var button_row = get_node_or_null("BottomPanel/ButtonRow")
	if button_row != null:
		var retreat_btn = Button.new()
		retreat_btn.name = "RetreatButton"
		retreat_btn.text = "Retreat"
		retreat_btn.custom_minimum_size = Vector2(100, 35)
		retreat_btn.focus_mode = Control.FOCUS_ALL
		retreat_btn.pressed.connect(_on_retreat_pressed)
		button_row.add_child(retreat_btn)
...
func _process(delta: float) -> void:
	...
	# Sprite animation: advance all active animators
	for uid in _unit_animators:
		_unit_animators[uid].advance(delta)
...
func _start_encounter() -> void:
	...
	_combat_controller.move_selection_required.connect(_on_move_selection_required)  # Grid Combat v1: Movement
	_combat_controller.unit_moved.connect(_on_unit_moved)  # Grid Combat v1: Movement
	_combat_controller.placement_phase_started.connect(_on_placement_phase_started)  # Phase 8: Hero placement
	_combat_controller.placement_confirmed.connect(_on_placement_confirmed)  # Phase 8: Placement done
	...
	# Phase 8: skip_placement=false to enable hero placement phase before combat
	_combat_controller.initialize_combat(hero_ids, enemy_ids, rng, combat_mod, false)
	...
	# Phase 8: If placement phase is active, don't start turns yet — wait for confirm
	if _combat_controller.is_placement_phase():
		return  # _on_placement_phase_started will handle UI setup
...
func _step_turn() -> void:
	...
	# Deferred UI refresh — gives sprite animations 0.35s to play before rebuilding
	if not _refresh_pending:
		_refresh_pending = true
		get_tree().create_timer(0.35).timeout.connect(_deferred_refresh)
...
## Lightweight HP/status update without destroying display nodes.
## Used after each action so sprite animations and tweens can keep playing.
func _update_unit_values() -> void:
	var snapshot = _combat_controller.get_units_snapshot()
	for unit_data in snapshot["player"] + snapshot["enemy"]:
		var uid: String = unit_data["id"]
		# Update HP bar value + label text
		if _unit_hp_labels.has(uid):
			var hp_label: Label = _unit_hp_labels[uid]
			if is_instance_valid(hp_label):
				...
				# Update the ProgressBar value (hp_label's parent)
				var hp_bar = hp_label.get_parent()
...
# Guard flag: prevents overlapping deferred refreshes
...
func _refresh_all_panels() -> void:
	...
	_unit_animators.clear()  # Sprite animation: Clear animators (rebuilt with displays)
...
## Grid Combat v1: Unified grid refresh — renders all units on the 8x4 grid.
## pos.x from snapshot = unified column (0-7), pos.y = unified row (0-3).
func _refresh_grid(player_units: Array, enemy_units: Array) -> void:
	# Stop all idle animations before clearing
	_stop_all_idle_bobs()
	# Clear all 32 cells
	_clear_all_slots()

	# Build lookup: "col,row" -> unit_data (unified grid coordinates)
	var occupied_cells: Dictionary = {}

	# Player units (typically cols 0-1)
	for unit_data in player_units:
		var pos = unit_data.get("pos", {})
		var col = clampi(pos.get("x", 0), 0, GRID_COLS - 1)
		var row = clampi(pos.get("y", 0), 0, GRID_ROWS - 1)
		var key = "%d,%d" % [col, row]
		if occupied_cells.has(key):
			push_warning("[GridCombatScene] Duplicate cell %s: %s overwrites %s" % [
				key, unit_data.get("id", "?"), occupied_cells[key].get("id", "?")])
		occupied_cells[key] = unit_data

	# Enemy units (typically cols 6-7) — skip during placement phase (fog of war)
	if not _placement_phase_active:
		for unit_data in enemy_units:
			var pos = unit_data.get("pos", {})
			var col = clampi(pos.get("x", 6), 0, GRID_COLS - 1)
			var row = clampi(pos.get("y", 0), 0, GRID_ROWS - 1)
			var key = "%d,%d" % [col, row]
			if occupied_cells.has(key):
				push_warning("[GridCombatScene] Duplicate cell %s: %s overwrites %s" % [
	...
	for col in range(GRID_COLS):
		for row in range(GRID_ROWS):
			...
			if occupied_cells.has(key):
				...
				# Grid Combat v1: Start idle bob for alive units
				if unit_data.get("is_alive", true):
					_start_idle_bob(unit_data["id"])
			else:
				# Phase 8: During placement, fog on enemy half (cols 4-7), clickable on player half (cols 0-3)
				if _placement_phase_active and col >= 4:
					cell.add_child(_create_fog_placeholder(col))
				elif _placement_phase_active and col <= 3:
					# Player half tile — make clickable for placement
					var empty_tile = _create_empty_slot_placeholder()
					empty_tile.mouse_filter = Control.MOUSE_FILTER_STOP
					empty_tile.gui_input.connect(_on_placement_empty_tile_input.bind(Vector2i(col, row)))
					cell.add_child(empty_tile)
				else:
...
## Legacy compatibility stubs for any remaining call sites.
func _refresh_party_panel(_units: Array) -> void:
	pass  # Handled by _refresh_grid()

func _refresh_enemy_panel(_units: Array) -> void:
	pass  # Handled by _refresh_grid()


func _get_or_create_card_background(class_id: String) -> Control:
	# Try texture cache first
	if _card_texture_cache.has(class_id):
		var cached_tex = _card_texture_cache[class_id]
		if cached_tex != null:
			var tex_rect = TextureRect.new()
			tex_rect.texture = cached_tex
			tex_rect.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
			tex_rect.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_COVERED
			tex_rect.modulate = Color(1, 1, 1, 0.85)
			return tex_rect
		# cached_tex is null = no texture file, fall through to procedural

	# Try loading texture from Assets/UI/Cards/
	if not _card_texture_cache.has(class_id):
		var tex_path: String = "res://Assets/UI/Cards/card_%s.png" % class_id
		if ResourceLoader.exists(tex_path):
			var tex = load(tex_path)
			_card_texture_cache[class_id] = tex
			if tex != null:
				var tex_rect = TextureRect.new()
			...
			_card_texture_cache[class_id] = null

	# Procedural fallback: gradient + border using card_palette
	var class_data = DataRegistry.get_class_data(class_id)
	if class_data == null or class_data.card_palette.is_empty():
		return null

	var palette: Dictionary = class_data.card_palette
	var bg_primary: Color = Color(palette.get("bg_primary", "#222222"))
	var bg_secondary: Color = Color(palette.get("bg_secondary", "#111111"))
	var border_color: Color = Color(palette.get("border_primary", "#444444"))
	var accent_color: Color = Color(palette.get("border_accent", "#666666"))
	...
	# Simulate gradient: add a semi-transparent ColorRect at bottom half
	panel.add_theme_stylebox_override("panel", style)

	var gradient_overlay = ColorRect.new()
	gradient_overlay.color = Color(bg_secondary.r, bg_secondary.g, bg_secondary.b, 0.5)
	...
	panel.add_child(gradient_overlay)

	# Accent stripe at top (thin colored bar)
	var accent_bar = ColorRect.new()
	accent_bar.color = Color(accent_color.r, accent_color.g, accent_color.b, 0.6)
	accent_bar.custom_minimum_size = Vector2(0, 3)
	accent_bar.set_anchors_preset(Control.PRESET_TOP_WIDE)
	accent_bar.size = Vector2(0, 3)
...
func _create_unit_display(unit_data: Dictionary) -> Control:
	# Grid Combat v1: Compact VERTICAL card that fits inside ~110x100 grid cell
	...
	wrapper.add_theme_constant_override("margin_top", 1)
	wrapper.add_theme_constant_override("margin_bottom", 1)
	...
	# --- Card background (class-themed, player heroes only) ---
	if unit_data["team"] == "player":
		var class_id: String = unit_data.get("class_id", "")
		if class_id != "":
			var card_bg: Control = _get_or_create_card_background(class_id)
			if card_bg != null:
	...
	# Vertical layout: name → portrait → stats → HP bar → badges
	var card_vbox = VBoxContainer.new()
	card_vbox.add_theme_constant_override("separation", 1)
	card_vbox.name = "UnitContent_%s" % unit_data["id"]
	card_vbox.size_flags_horizontal = Control.SIZE_EXPAND_FILL

	# --- Name label (truncated to fit cell) ---
	...
	name_label.add_theme_font_size_override("font_size", GameContext.fs(11))
	...
	name_label.tooltip_text = _build_unit_card_tooltip(unit_data)
	name_label.gui_input.connect(func(event: InputEvent):
		if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT and event.ctrl_pressed:
			_show_pinned_tooltip(name_label.tooltip_text, name_label)
			get_viewport().set_input_as_handled()
	)
	# Text shadow for readability over card backgrounds (player heroes)
	if unit_data["team"] == "player":
	...
	# --- Portrait (64x64 centered) — click target for target selection ---
	var portrait_center = CenterContainer.new()
	portrait_center.mouse_filter = Control.MOUSE_FILTER_PASS
	...
	var portrait_path: String = unit_data.get("portrait_path", "")
	if portrait_path != "":
		var tex = load(portrait_path)
		if tex != null:
			portrait_rect.texture = tex
	# Sprite animation: if unit has a sprite folder, create animator for frame-by-frame animation
	var sprite_folder: String = unit_data.get("sprite_folder", "")
	if sprite_folder != "":
		var sf: SpriteFrames = SpriteAnimationLoader.load_sprite_frames(sprite_folder)
		if sf != null:
			var animator = HeroSpriteAnimator.new(sf, portrait_rect)
			_unit_animators[unit_data["id"]] = animator
			# 32x32 sprites displayed at 64x64 with nearest-neighbor scaling
	...
	# --- Compact stats line: A:8 D:21 S:3 ---
	var stats_label = Label.new()
	stats_label.name = "StatsRow"
	var atk_val: int = int(unit_data.get("attack", 0))
	var def_val: int = int(unit_data.get("defense", 0))
	var spd_val: int = int(unit_data.get("speed", 0))
	stats_label.text = "A:%d D:%d S:%d" % [atk_val, def_val, spd_val]
	stats_label.add_theme_font_size_override("font_size", GameContext.fs(10))
	stats_label.add_theme_color_override("font_color", Color(0.75, 0.72, 0.65, 0.9))
	stats_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	stats_label.mouse_filter = Control.MOUSE_FILTER_STOP
	stats_label.tooltip_text = _build_combat_card_stat_tooltip("attack", "ATK", unit_data)
	if unit_data["team"] == "player":
		stats_label.add_theme_color_override("font_shadow_color", Color(0, 0, 0, 0.7))
	...
	# Register stat labels for live updates (use a dict with single label ref)
	_unit_stat_labels[unit_data["id"]] = {"_compact": stats_label, "attack": stats_label, "defense": stats_label, "speed": stats_label}

	# --- HP bar (thin, full width) ---
	...
	if shield_val > 0:
		...
		hp_label.text = "%d/%d" % [int(unit_data["hp"]), int(unit_data["max_hp"])]
	hp_label.add_theme_font_size_override("font_size", GameContext.fs(10))
	...
	if shield_bar_val > 0:
		...
		s_bg.set_corner_radius_all(1)
		shield_bar.add_theme_stylebox_override("background", s_bg)
		var s_fill = StyleBoxFlat.new()
		s_fill.bg_color = Color(0.3, 0.7, 1.0, 0.9)
		s_fill.set_corner_radius_all(1)
		shield_bar.add_theme_stylebox_override("fill", s_fill)
		card_vbox.add_child(shield_bar)

	# Class label (player units only — fills space below HP bar)
	var cid: String = unit_data.get("class_id", "")
	if cid != "" and unit_data["team"] == "player":
		var class_label = Label.new()
		class_label.name = "ClassLabel"
		class_label.text = cid.replace("_", " ").capitalize()
		class_label.add_theme_font_override("font", KENNEY_FUTURE_FONT)
		class_label.add_theme_font_size_override("font_size", GameContext.fs(12))
		class_label.add_theme_color_override("font_color", Color(0.85, 0.75, 0.55, 0.95))
		class_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		class_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
		class_label.size_flags_vertical = Control.SIZE_EXPAND_FILL
		class_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
		card_vbox.add_child(class_label)

	# --- Status + buff badges (single combined row) ---
	var unit_id: String = unit_data["id"]
	var status_badge_row = HBoxContainer.new()
	status_badge_row.name = "StatusBadgeRow"
	status_badge_row.add_theme_constant_override("separation", 2)
	status_badge_row.alignment = BoxContainer.ALIGNMENT_CENTER
	_populate_status_badges(status_badge_row, unit_data.get("active_statuses_v1", []), unit_id)
	card_vbox.add_child(status_badge_row)

	var buff_badge_row = HBoxContainer.new()
	buff_badge_row.name = "BuffBadgeRow"
	buff_badge_row.add_theme_constant_override("separation", 2)
	buff_badge_row.alignment = BoxContainer.ALIGNMENT_CENTER
	_populate_buff_badges(buff_badge_row, unit_data.get("active_buffs_v1", []), unit_id)
	card_vbox.add_child(buff_badge_row)
	...
	# D2: Turn counter (player units only — tucked into tooltip, not shown inline)
	if unit_data["team"] == "player":
		var turn_label = Label.new()
		turn_label.name = "TurnCounter"
		turn_label.text = ""
		turn_label.add_theme_font_size_override("font_size", GameContext.fs(7))
		...
		card_vbox.add_child(turn_label)
...
# ============================================================================
# FLEE / RETREAT DIALOG — Offered on hero death or voluntary retreat
# ============================================================================

func _show_flee_dialog(fallen_name: String, is_retreat: bool = false) -> void:
	...
	var title = Label.new()
	if is_retreat:
		title.text = "Retreat from Combat?"
	...
	var desc = Label.new()
	if is_retreat:
		desc.text = "Flee the dungeon and return to town?"
	else:
	...
	flee_btn.text = "Retreat" if is_retreat else "Flee"
	...
	print("[Flee] Dialog shown — %s" % ("retreat requested" if is_retreat else "%s has fallen" % fallen_name))

## Handler for the Retreat button in combat UI.
...
func _on_action_performed(action: CombatAction) -> void:
	...
	# Grid Combat v1: Tween animations
	match action.action_type:
		CombatAction.ActionType.BASIC_ATTACK, CombatAction.ActionType.WEAPON_ABILITY:
			_play_attack_tween(action.actor_id, action.target_id)
			if action.damage_dealt > 0 and not action.was_evaded:
				_play_hurt_tween(action.target_id)
		CombatAction.ActionType.CLASS_ABILITY, CombatAction.ActionType.EQUIPMENT_ABILITY:
			_play_cast_tween(action.actor_id)
			if action.damage_dealt > 0 and not action.was_evaded:
				_play_hurt_tween(action.target_id)
			elif action.healing_done > 0:
				_play_heal_tween(action.target_id)
		CombatAction.ActionType.DEATH:
			_play_death_tween(action.actor_id)
		CombatAction.ActionType.DOOM_TRIGGER:
			if action.damage_dealt > 0:
				_play_hurt_tween(action.target_id)
	...
	# Lightweight HP update — don't rebuild nodes so sprite animations can play
	_update_unit_values()
...
func _create_v19b_ui() -> void:
	...
	# Style log overlay background (RPG pack)
...
func _create_action_panel() -> void:
	...
	# Grid Combat v1: Move button (free movement, shown before attack options)
	_btn_move = Button.new()
	_btn_move.text = "[M] Move"
	_btn_move.custom_minimum_size = Vector2(80, 34)
	_btn_move.focus_mode = Control.FOCUS_ALL
	_btn_move.pressed.connect(_on_move_pressed)
	_btn_move.visible = false
	hbox.add_child(_btn_move)
...
func _on_player_input_required(unit: CombatUnit, available_actions: Array) -> void:
	...
	for action_info in available_actions:
		...
		if atype == "move":
			if _btn_move != null:
				_btn_move.visible = true
				_btn_move.disabled = not action_info.enabled
				_btn_move.tooltip_text = "Move to a new tile (free action, once per turn)"
		elif atype == "basic":
...
func _enter_target_selection_mode(valid_targets: Array) -> void:
	...
	for unit_id in _unit_displays.keys():
		...
		if unit_id in _valid_target_ids:
			# Add highlight to the CELL (PanelContainer parent), not the wrapper.
			# This ensures it sits on top of ALL content and catches clicks on the full tile.
			var cell = display.get_parent()
			var highlight = cell.get_node_or_null("TargetHighlight")
			if highlight == null:
			...
			# Connect click and hover handlers to highlight (covers full cell)
			for connection in highlight.gui_input.get_connections():
				if connection.callable.get_method() == "_on_unit_clicked":
					highlight.gui_input.disconnect(connection.callable)
			for connection in highlight.mouse_entered.get_connections():
				if connection.callable.get_method() == "_on_target_mouse_entered":
					highlight.mouse_entered.disconnect(connection.callable)
			for connection in highlight.mouse_exited.get_connections():
				if connection.callable.get_method() == "_on_target_mouse_exited":
					highlight.mouse_exited.disconnect(connection.callable)

			highlight.gui_input.connect(_on_unit_clicked.bind(unit_id))
			highlight.mouse_entered.connect(_on_target_mouse_entered.bind(unit_id))
			highlight.mouse_exited.connect(_on_target_mouse_exited.bind(unit_id))
		else:
...
func _exit_target_selection_mode() -> void:
	...
	_valid_target_ids.clear()
	# Also clear any movement highlights
	...
	for unit_id in _unit_displays.keys():
		...
		# Disconnect target selection handlers from TargetHighlight + hide it
		var cell = display.get_parent()
		var highlight = cell.get_node_or_null("TargetHighlight") if cell else null
		if highlight:
			for connection in highlight.gui_input.get_connections():
				if connection.callable.get_method() == "_on_unit_clicked":
					highlight.gui_input.disconnect(connection.callable)
...
# ============================================================================
# GRID COMBAT v1: Movement Selection
# ============================================================================

## Handle Move button press — enter tile selection mode for free movement.
func _on_move_pressed() -> void:
	print("[UI] Move button pressed")
	_combat_controller.submit_player_action("move")


## Handle move_selection_required signal — show reachable tiles.
func _on_move_selection_required(unit: CombatUnit, reachable_tiles: Array) -> void:
	print("[UI] move_selection_required for %s, %d reachable tiles" % [unit.display_name, reachable_tiles.size()])
	_move_selection_active = true
	_valid_move_tiles = reachable_tiles

	# Hide action panel during tile selection, show cancel
	_action_panel.visible = true
	_btn_basic.visible = false
	_btn_ability_a.visible = false
	_btn_ability_b.visible = false
	_btn_pass.visible = false
	_btn_use_item.visible = false
	...
	if _btn_move != null:
		_btn_move.visible = false
	_btn_cancel.visible = true
	_action_label.text = "Select destination tile:"

	# Highlight reachable tiles with blue overlay
	_clear_move_highlights()
	for tile_pos in reachable_tiles:
		var cell = _get_cell_at(tile_pos.y, tile_pos.x)
		if cell == null:
			continue
		...
		highlight.color = Color(0.2, 0.5, 1.0, 0.3)  # Blue tint
		highlight.mouse_filter = Control.MOUSE_FILTER_STOP  # Catch clicks
		highlight.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		highlight.size_flags_vertical = Control.SIZE_EXPAND_FILL
		highlight.custom_minimum_size = cell.custom_minimum_size

		# Connect click handler
		highlight.gui_input.connect(_on_move_tile_clicked.bind(tile_pos))
		highlight.mouse_entered.connect(_on_move_tile_hover_entered.bind(cell))
		highlight.mouse_exited.connect(_on_move_tile_hover_exited.bind(cell))

		cell.add_child(highlight)
		_move_tile_highlights.append(highlight)


## Handle click on a reachable tile during movement selection.
func _on_move_tile_clicked(event: InputEvent, tile_pos: Vector2i) -> void:
	if not _move_selection_active:
		return
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		print("[UI] Move tile clicked: (%d, %d)" % [tile_pos.x, tile_pos.y])
		_exit_move_selection_mode()
		_combat_controller.submit_player_move(tile_pos)


## Visual feedback: highlight tile on hover.
func _on_move_tile_hover_entered(cell: PanelContainer) -> void:
	var highlight = cell.get_node_or_null("MoveHighlight")
	if highlight:
		highlight.color = Color(0.3, 0.6, 1.0, 0.5)  # Brighter on hover


## Visual feedback: dim tile on hover exit.
func _on_move_tile_hover_exited(cell: PanelContainer) -> void:
	var highlight = cell.get_node_or_null("MoveHighlight")
	if highlight:
		highlight.color = Color(0.2, 0.5, 1.0, 0.3)  # Back to normal


## Handle unit_moved signal — refresh grid display after movement.
func _on_unit_moved(unit: CombatUnit, from_pos: Vector2i, to_pos: Vector2i) -> void:
	print("[UI] unit_moved: %s from (%d,%d) to (%d,%d)" % [
		unit.display_name, from_pos.x, from_pos.y, to_pos.x, to_pos.y])
	# Refresh the entire grid to show the unit in its new position
	if _combat_controller != null:
		var snapshot = _combat_controller.get_units_snapshot()
		_refresh_grid(snapshot["player"], snapshot["enemy"])


## Exit movement tile selection mode — remove highlights and reset state.
func _exit_move_selection_mode() -> void:
	_move_selection_active = false
	_valid_move_tiles.clear()
	_clear_move_highlights()


## Remove all movement highlight overlays from the grid.
func _clear_move_highlights() -> void:
	for highlight in _move_tile_highlights:
		if is_instance_valid(highlight):
			highlight.queue_free()
	_move_tile_highlights.clear()


# ============================================================================
# GRID COMBAT v1: TWEEN ANIMATIONS (Phase 7)
# ============================================================================

## Play attack tween on the actor's portrait: lunge toward target direction + return.
func _play_attack_tween(actor_id: String, target_id: String) -> void:
	var display = _unit_displays.get(actor_id)
	...
	# Determine lunge direction based on relative grid positions
	var lunge_x: float = 12.0  # Default: lunge right
	var actor_unit = _combat_controller.get_unit_by_id(actor_id) if _combat_controller else null
	var target_unit = _combat_controller.get_unit_by_id(target_id) if _combat_controller else null
	if actor_unit != null and target_unit != null:
		if target_unit.grid_x < actor_unit.grid_x:
			lunge_x = -12.0  # Lunge left toward target
		elif target_unit.grid_x == actor_unit.grid_x:
			lunge_x = 0.0  # Same column — use vertical
	var original_pos = portrait.position

	# Sprite animation: trigger attack frames alongside tween lunge
	if _unit_animators.has(actor_id):
		_unit_animators[actor_id].play("attack")

	var tween = create_tween()
	tween.tween_property(portrait, "position:x", original_pos.x + lunge_x, 0.1).set_ease(Tween.EASE_OUT)
	tween.tween_property(portrait, "position:x", original_pos.x, 0.15).set_ease(Tween.EASE_IN)


## Play hurt tween on a target: flash red + shake.
func _play_hurt_tween(target_id: String) -> void:
	var display = _unit_displays.get(target_id)
	if display == null or not is_instance_valid(display):
		return
	var portrait = display.find_child("Portrait", true, false)
	if portrait == null:
	...
	# Sprite animation: trigger hit frames alongside tween flash/shake
	if _unit_animators.has(target_id):
		_unit_animators[target_id].play("hit")

	var tween = create_tween()
	# Flash red
	tween.tween_property(portrait, "modulate", Color(1.0, 0.3, 0.3, 1.0), 0.05)
	# Shake
	tween.tween_property(portrait, "position:x", original_pos.x + 4, 0.03)
	tween.tween_property(portrait, "position:x", original_pos.x - 4, 0.03)
	tween.tween_property(portrait, "position:x", original_pos.x + 2, 0.03)
	tween.tween_property(portrait, "position:x", original_pos.x, 0.03)
	# Fade back to normal color
	tween.tween_property(portrait, "modulate", Color.WHITE, 0.15)


## Play death tween: tilt + fade out.
func _play_death_tween(unit_id: String) -> void:
	var display = _unit_displays.get(unit_id)
	if display == null or not is_instance_valid(display):
		return
	var portrait = display.find_child("Portrait", true, false)
	...
	_stop_idle_bob(unit_id)

	# Sprite animation: trigger death frames alongside tween tilt/fade
	if _unit_animators.has(unit_id):
		_unit_animators[unit_id].play("death")

	var tween = create_tween()
	tween.set_parallel(true)
	tween.tween_property(portrait, "rotation", deg_to_rad(30.0), 0.6).set_ease(Tween.EASE_IN)
	tween.tween_property(portrait, "modulate:a", 0.0, 0.6).set_ease(Tween.EASE_IN)


## Play heal tween: green glow + scale pulse.
func _play_heal_tween(target_id: String) -> void:
	var display = _unit_displays.get(target_id)
	if display == null or not is_instance_valid(display):
		return
	var portrait = display.find_child("Portrait", true, false)
	...
	var tween = create_tween()
	tween.tween_property(portrait, "modulate", Color(0.4, 1.0, 0.5, 1.0), 0.1)
	tween.tween_property(portrait, "scale", Vector2(1.1, 1.1), 0.15).set_ease(Tween.EASE_OUT)
	tween.tween_property(portrait, "scale", Vector2(1.0, 1.0), 0.15).set_ease(Tween.EASE_IN)
	tween.tween_property(portrait, "modulate", Color.WHITE, 0.2)


## Play cast tween: cyan glow + scale pulse (for ability use).
func _play_cast_tween(actor_id: String) -> void:
	var display = _unit_displays.get(actor_id)
	if display == null or not is_instance_valid(display):
		return
	var portrait = display.find_child("Portrait", true, false)
	...
	# Sprite animation: trigger cast frames alongside tween glow/pulse
	if _unit_animators.has(actor_id):
		_unit_animators[actor_id].play("cast")

	var tween = create_tween()
	tween.tween_property(portrait, "modulate", Color(0.5, 0.8, 1.0, 1.0), 0.1)
	tween.tween_property(portrait, "scale", Vector2(1.1, 1.1), 0.15).set_ease(Tween.EASE_OUT)
	tween.tween_property(portrait, "scale", Vector2(1.0, 1.0), 0.15).set_ease(Tween.EASE_IN)
	tween.tween_property(portrait, "modulate", Color.WHITE, 0.2)


## Start idle bob animation on a unit portrait (subtle up/down loop).
## Sprite-animated units use their idle animation instead of position bob.
func _start_idle_bob(unit_id: String) -> void:
	# Sprite-animated units already have idle animation — skip tween bob
	if _unit_animators.has(unit_id):
		return
	var display = _unit_displays.get(unit_id)
	if display == null or not is_instance_valid(display):
	...
	# Stop any existing idle bob
	_stop_idle_bob(unit_id)

	var base_y = portrait.position.y
	var tween = create_tween()
	tween.set_loops()
	tween.tween_property(portrait, "position:y", base_y - 2.0, 0.4).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(portrait, "position:y", base_y, 0.4).set_ease(Tween.EASE_IN_OUT)
	_idle_tweens[unit_id] = tween


## Stop idle bob animation on a unit.
func _stop_idle_bob(unit_id: String) -> void:
	if _idle_tweens.has(unit_id):
		var tween = _idle_tweens[unit_id]
		if tween != null and tween.is_valid():
			tween.kill()
		_idle_tweens.erase(unit_id)


## Stop all idle bob animations.
func _stop_all_idle_bobs() -> void:
	for unit_id in _idle_tweens.keys():
		_stop_idle_bob(unit_id)
...
func _on_unit_left_clicked(unit_id: String) -> void:
	# Phase 8: During placement, handle placement clicks instead of combat
	if _placement_phase_active:
		_on_placement_unit_clicked(unit_id)
		return
